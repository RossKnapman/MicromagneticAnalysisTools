...
def skyrmionNumber(m):
    ...
    toSum = np.einsum('ijk,ijk->ij', mReduced, np.cross(mdx, mdy))  # Element-wise dot product
...
def skyrmionNumberDensity(m, dx, dy, lengthUnits=None):
    ...
    d_dx = findiff.FinDiff(0, 1, 1, acc=8)  # args: axis, lattice constant, derivative order, accuracy
    d_dy = findiff.FinDiff(1, 1, 1, acc=8)  # args: axis, lattice constant, derivative order, accuracy
...
def skyrmionCOMArray(directory, edgeCutXFrac = 0.1, edgeCutYFrac = 0.1, zIndex = 0):
    ...
    filesToScan = getFilesToScan(directory)
    dx, dy = sampleDiscretisation(directory)[:2]
    COM = np.zeros((len(filesToScan), 2))

    for i in range(len(filesToScan)):
        print('Calculating skyrmion position', i, 'of', len(filesToScan) - 1, end='\r')
        COM[i] = skyrmionCOM(directory, filesToScan[i], edgeCutXFrac, edgeCutYFrac, dx, dy, zIndex)
...
def skyrmionCOMCoMoving(directory, inFile, dx, dy, zIndex, guessX, guessY, boxSize=None):
    ...
    if boxSize:  # The size of the box around the guessed COM in nm to reduce computational resources rather than integration over entire system. This does not really seem to give much of a performance boost.
        boxSizeIdx = int(np.round(m.shape[0] * boxSize / Lx))  # Convert size to array indices
...
def skyrmionCOMArrayCoMoving(directory, zIndex = 0, boxSize = None, startFile=None, endFile=None):
    ...
    filesToScan = getFilesToScan(directory, startFile=startFile, endFile=endFile)
    dx, dy = sampleDiscretisation(directory)[:2]
    COM = np.zeros((len(filesToScan), 2))
    ...
    for i in range(len(filesToScan)):
        print('Calculating skyrmion position', i, 'of', len(filesToScan) - 1, end='\r')
        COM[i] = skyrmionCOMCoMoving(directory, filesToScan[i], dx, dy, zIndex, guessX, guessY, boxSize)
        guessX, guessY = COM[i]
...
def getRampIdx(directory, component):
    ...
    currentArray = simulationCurrentArray(directory, component)
...
def NskArray(directory, zIndex = 0, startFile = None, endFile = None, calculateFromX = None, calculateToX = None, calculateFromY = None, calculateToY = None, startXFrac = 0, endXFrac = 1, startYFrac = 0, endYFrac = 1):
    ...
    for i in range(len(filesToScan)):
        ...
        m = df.Field.fromfile(directory + '/' + filesToScan[i]).array[:, :, zIndex]
